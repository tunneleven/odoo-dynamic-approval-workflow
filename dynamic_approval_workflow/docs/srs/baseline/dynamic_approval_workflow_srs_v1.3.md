...
10. `FR-090`: Binding configuration shall support enforcement modes: `orm_enforced` (recommended default), `hybrid`, and optional `ui_only` for explicitly approved low-risk/simple models.
11. `FR-091`: For `orm_enforced` and `hybrid` modes, gate enforcement shall execute in target business methods to cover non-UI channels (RPC, import, automated actions, cron, server actions).
12. `FR-092`: `ui_only` mode shall require explicit Workflow Admin risk acknowledgment and shall be disallowed for bindings tagged `compliance_critical`.
13. `FR-093`: Binding configuration shall allow selecting a post-approval callback target (`model.method` / Odoo button method) executed when terminal approval is reached (example: `sale.order.action_confirm`).
14. `FR-094`: Callback configuration shall be validated at publish time for model existence, callable existence, ACL compatibility, and idempotency policy.
15. `FR-095`: Post-approval callback failures shall create incident records, preserve audit evidence, and expose controlled retry/recovery actions.
...
3. `FR-045`: The system shall store immutable evidence fields: `evidence_type` (`human_signature` or `system_attestation`), signer/actor identity, timestamp, method, and reference/hash.
4. `FR-046`: Audit logs and reports shall distinguish standard approvals, human-signature approvals, and system attestation outcomes.
5. `FR-084`: For steps where `sign_required = true`, timeout policy `auto-approve` may create a system-triggered attestation artifact using a dedicated non-human signer identity, explicit reason code (`timeout_auto_approve`), and cryptographic evidence; this evidence shall be labeled `system_attestation` and must not be presented as a human signature.
6. `FR-085`: System attestation shall be explicitly enabled per step by Workflow Admin policy; otherwise `sign_required = true` steps shall restrict timeout policies to `auto-reject` or `escalate-only`.
7. `FR-096`: For steps tagged `legal_human_signature_required`, timeout `auto-approve` shall be disallowed regardless of system attestation policy.
...
## 7.14 Definition Activation and Rollback Precedence
1. `FR-086`: Version selection for new instances shall resolve to the latest `published` definition version that is active for the instance start timestamp and company scope.
2. `FR-087`: In-flight instances shall remain pinned to their start-time definition version; effective-date changes and rollback actions shall only affect newly started instances.
3. `FR-088`: Rollback activation shall require explicit `effective_from_utc` and operator reason, and shall emit immutable audit events for activation and supersession.
4. `FR-089`: If overlapping activation windows exist for the same binding scope, precedence shall be deterministic: highest rollout specificity, then latest publish timestamp; unresolved ties shall block activation until conflict is corrected.
...
7. Binding:
`workflow_binding { id, model, action_key, enforcement_mode, post_approve_callback, callback_user_mode, rollout_scope, compliance_critical }`
...
8. `cancel_instance(instance_id, reason, idempotency_key)`
9. `reassign_task(task_id, assignee_ref, reason, idempotency_key)`
10. All mutating operations shall be idempotent by `idempotency_key` within operation scope.
11. If an `expected_*_version` token is stale, the adapter shall return deterministic conflict response with no state mutation.
12. A duplicate mutating call with same `idempotency_key` shall return original outcome and correlation reference without creating duplicate side effects.
13. `execute_post_approval_callback(instance_id, callback_ref, idempotency_key)`
...
2. Required headers: `X-Workflow-Event-Id`, `X-Workflow-Timestamp`, `X-Workflow-Signature-Version`, `X-Workflow-Signature`.
3. Canonical signing input shall be `timestamp + "." + event_id + "." + sha256(canonical_json_payload)`, where canonical JSON is RFC-8785-compatible UTF-8 serialization.
4. Key management: per-endpoint secret with rotation support and overlap window.
5. Verification contract: consumer validates signature and timestamp within a `300s` replay window; duplicate `event_id` must be ignored idempotently.
6. Security posture: invalid signature or expired timestamp results in consumer-side rejection and producer retry policy handling.
...
3. `NFR-003`: Runtime shall support baseline 1,000 approvals per rolling 24-hour window; burst factor `5x` is defined as at least `4 approvals/minute` sustained for `15 minutes` (minimum `60 approvals/15m`), with at least 500 concurrent active instances.
...
16. `NFR-016`: Mutating runtime adapter operations shall be exactly-once effective under at-least-once delivery conditions when valid `idempotency_key` is provided.
17. `NFR-017`: For bindings in `orm_enforced` or `hybrid` mode, gate enforcement shall be consistent across UI and non-UI invocation channels.
...
23. Signed-required step with timeout `auto-approve` produces valid `system_attestation` evidence with dedicated signer identity and reason code, and is clearly differentiated from `human_signature`.
24. Version activation matrix verifies precedence for effective-date, rollback, and rollout scope conflicts; in-flight instances stay pinned.
25. Duplicate mutating adapter calls with same `idempotency_key` produce no duplicate side effects and return stable response contract.
26. Webhook verification rejects signatures outside `300s` replay window and accepts RFC-8785 canonical payload signatures.
27. In `orm_enforced` mode, blocked action invocation through RPC/import/automated action/cron/server action is denied consistently.
28. `ui_only` mode requires explicit risk acknowledgment and is rejected for `compliance_critical` bindings.
29. Final approval on `sale.order` binding executes configured callback `action_confirm` exactly once and records callback audit trail.
30. Callback execution failure is incidented and supports controlled retry without duplicate business side effects.
...
5. Risk: Misuse of system-triggered signature in regulated processes.
Mitigation: explicit per-step opt-in, dedicated technical signer identity, mandatory reason code, and compliance-facing audit reporting.
...
## 17. Version Lifecycle State Table Annex

## 17.1 Definition Version Registry (Conceptual)
| Field | Type | Required | Description |
|---|---|---|---|
| `definition_key` | `text` | Yes | Stable workflow identifier (for example `po_approval`). |
| `version` | `int` | Yes | Monotonic version number for immutable published artifacts. |
| `status` | `enum(draft,published,archived)` | Yes | Lifecycle state; only `published` is start-eligible. |
| `published_at_utc` | `timestamptz` | Yes | Immutable publish timestamp. |
| `active_from_utc` | `timestamptz` | Yes | Activation start timestamp for new instances. |
| `active_to_utc` | `timestamptz/null` | No | Optional end of activation window. |
| `rollout_scope` | `jsonb` | Yes | Scope filter (`company/group/domain`). |
| `supersedes_version` | `int/null` | No | Previous active version this activation supersedes. |
| `activation_event_id` | `uuid` | Yes | Immutable event id for activation/rollback action. |
| `activation_reason_code` | `text` | Yes | Reason (`initial_release`, `policy_update`, `rollback_incident`). |
| `activated_by` | `user_ref` | Yes | Operator identity (admin/system). |
| `bpmn_hash` | `char(64)` | Yes | Canonical `bpmn_xml` hash bound to compiled metadata. |

## 17.2 Activation Event Log Example
| Event Time (UTC) | definition_key | version | active_from_utc | active_to_utc | supersedes_version | reason_code | Notes |
|---|---|---:|---|---|---:|---|---|
| `2026-03-01T09:00:00Z` | `po_approval` | 1 | `2026-03-01T09:00:00Z` | `2026-03-15T09:00:00Z` | `null` | `initial_release` | First production activation. |
| `2026-03-15T08:30:00Z` | `po_approval` | 2 | `2026-03-15T09:00:00Z` | `2026-03-20T10:00:00Z` | 1 | `policy_update` | Added CFO condition. |
| `2026-03-20T09:30:00Z` | `po_approval` | 3 | `2026-03-20T10:00:00Z` | `2026-03-20T14:30:00Z` | 2 | `policy_update` | Tightened rejection rule; later reverted. |
| `2026-03-20T14:20:00Z` | `po_approval` | 2 | `2026-03-20T14:30:00Z` | `null` | 3 | `rollback_incident` | Rollback is a new activation event, not mutation of v3. |

## 17.3 Instance Resolution and Pinning Example
| Instance ID | instance_started_at_utc | Resolved Version at Start | Version Used Until End | Outcome |
|---|---|---:|---:|---|
| `POI-1001` | `2026-03-15T08:55:00Z` | 1 | 1 | Started before v2 activation; remains on v1. |
| `POI-1002` | `2026-03-15T09:05:00Z` | 2 | 2 | Started after v2 activation; uses v2. |
| `POI-1003` | `2026-03-20T10:05:00Z` | 3 | 3 | Started while v3 active; remains on v3 even after rollback. |
| `POI-1004` | `2026-03-20T14:35:00Z` | 2 | 2 | Started after rollback activation; uses re-activated v2. |

## 17.4 Deterministic Selection Rule (At Instance Start)
1. Filter to records with `status = published`, matching rollout scope, and `active_from_utc <= start_time < active_to_utc` (`active_to_utc = null` means open-ended).
2. Prefer highest rollout specificity (`company` > `group` > `global`).
3. If still tied, select greatest `published_at_utc`.
4. If tie still remains, reject instance start and raise configuration conflict incident.

## 18. Odoo Implementation Pattern Annex (Enforcement and Callback)

## 18.1 Recommended Pattern
1. Keep workflow logic centralized in a shared mixin/service.
2. Add thin model-level hooks only for guarded business methods.
3. Use binding config to select `enforcement_mode` and `post_approve_callback`.
4. Allow `ui_only` only for explicitly approved low-risk/simple models.

## 18.2 Shared Mixin Skeleton (`orm_enforced` / `hybrid`)
```python
...
class WorkflowApprovalMixin(models.AbstractModel):
    ...
    _description = "Workflow Approval Enforcement Mixin"

    def _workflow_check_gate(self, action_key):
        self.ensure_one()
        gate = self.env["workflow.engine"].get_gate_state(
            model=self._name,
            res_id=self.id,
            action_key=action_key,
        )
        if gate.get("state") == "blocked":
            raise UserError(_("Approval is required before this action."))
        return gate
```

## 18.3 Example: `sale.order` with `action_confirm`
```python
from odoo import models


class SaleOrder(models.Model):
    _inherit = ["sale.order", "workflow.approval.mixin"]

    def action_confirm(self):
        for order in self:
            order._workflow_check_gate("action_confirm")
        return super().action_confirm()
```

## 18.4 Example Binding Records
1. `orm_enforced` example:
`{ model: "sale.order", action_key: "action_confirm", enforcement_mode: "orm_enforced", post_approve_callback: "sale.order.action_confirm" }`
2. `ui_only` example (simple model only, not compliance-critical):
`{ model: "x_simple.request", action_key: "submit", enforcement_mode: "ui_only", risk_ack_by: "<admin>" }`

## 18.5 Post-Approval Callback Dispatch (Terminal Node)
```python
def _on_instance_terminal_approved(self, instance):
    binding = instance.binding_id
    callback_ref = binding.post_approve_callback  # e.g. sale.order.action_confirm
    if not callback_ref:
        return
    self.env["workflow.engine"].execute_post_approval_callback(
        instance_id=instance.id,
        callback_ref=callback_ref,
        idempotency_key=f"wf-callback:{instance.id}:{binding.id}",
    )
```
